...
const titlesText = computed<string[]>(() =>
  (awesome?.layout?.welcome?.title || awesome?.name || 'Camrent360')
    .replaceAll('&nbsp;', '[space]')
    .split(' ')
    ...
    <LayoutPageSection class="flex-1 flex">
      <div class="flex-1 flex flex-col items-center justify-center">
        ...
        <div class="px-4 mt-6 text-center max-w-[500px] md:max-w-[600px]">
          {{ awesome?.description || 'one stop for all your kit.' }}
        </div>
        <div v-if="showAlert" class="mt-4 w-auto text-center text-white bg-gray-800 rounded px-4 py-1 text-sm">
          create file "~/pages/index.vue" to replace this page
        </div>
        <div class="flex space-x-4 ml-2 mt-8 justify-center md:justify-start">
          <AwesomeButton size="lg" :text="parseMenuTitle(
            awesome?.layout?.welcome?.primaryActionButton?.title ||
            'products'
          )
            " :to="parseMenuRoute(
    awesome?.layout?.welcome?.primaryActionButton?.to || '/products'
  )
